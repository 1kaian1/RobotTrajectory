...
```
When running and RViz is started, publish new 2D Nav Goal in RViz. The global_planner will then calculate the shortes path to that goal and publish it, so the local_planner can simulate, create and choose a trajectory which is then executed. 
## Final presentation
